...
import { SimulatedAccountsPanel } from "./simulated-accounts-panel";
...
// Manage User approval status, derived from existing kkri_users fields. There is no
...
function bucketOf(u: User): UserBucket {
    ...
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      <header className="flex flex-col gap-2 border-b border-white/5 pb-4">
        ...
        <p className="text-[13px] text-ink-muted">
          Approval untuk user (Pembina KKRI) yang register via mobile app, atau dibuat manual.
          Alur: user register → berstatus <em>pending</em> → muncul di sini → admin approve / reject → baru bisa aktif.
          Klik baris untuk lihat detail (riwayat GPS, laporan, sekolah binaan).
        </p>
      </header>

      {/* Self-registered accounts pending review — sourced from localStorage,
          see lib/auth-sim.ts. Renders nothing until someone has registered. */}
      <SimulatedAccountsPanel />
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari nama, email, HP, NRP, unit_id…"
      ...
      {users.length === 0 ? (
        <div className="panel p-10 text-center">
          ...
          <div className="mt-1 text-[12px] text-ink-subtle">Menunggu integrasi database/auth.</div>
        </div>
      ) : (
        <div className="panel overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-[12px]">
              ...
              <tbody>
                {filtered.length === 0 && (
                  <tr><td colSpan={6} className="px-3 py-8 text-center text-ink-subtle">Tidak ada user cocok dengan filter.</td></tr>
                )}
                {filtered.map(u => (
    ...
    <>
      <tr className={`border-t border-white/5 hover:bg-white/[0.02] align-top ${bucket === "pending" ? "bg-warn/5" : ""}`}>
        <td className="px-3 py-2.5">
          <Link href={`/admin/users/${user.id}`} className="font-medium text-ink hover:text-accent-glow transition">{user.full_name}</Link>
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4"
      ...
      <div className="w-full max-w-md rounded-lg border border-white/10 bg-bg-soft p-5 shadow-2xl">
        <div className="mb-4 flex items-baseline justify-between">
          <h2 className="font-display text-xl font-bold text-ink">Tambah User Pembina</h2>
