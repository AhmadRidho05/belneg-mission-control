...
import SiswaDetailClient from "./siswa-detail-client";
...
export default async function SiswaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [latestAssessment, selfAssessments, learningPath, learningPhases, courseProgress, activityLog, badges] = await Promise.all([
    qGet<any>(
      `SELECT id, riasec_realistic AS r, riasec_investigative AS i,
       ...
       ORDER BY submitted_at DESC LIMIT 1`, [id]),
    qAll<any>(
      `SELECT sa.onet_skill_id AS element_id, sa.current_level, sa.target_level,
       ...
       WHERE sa.user_id = ?
       ORDER BY sa.gap_category, (sa.target_level - sa.current_level) DESC`, [id]),
    qGet<any>(
      `SELECT id, target_career_onet, generated_at FROM siswa_learning_paths
       WHERE user_id = ? ORDER BY generated_at DESC LIMIT 1`, [id]),
    qAll<any>(
      `SELECT ph.id, ph.phase_number, ph.title, ph.estimated_weeks
       FROM siswa_learning_phases ph
       JOIN siswa_learning_paths lp ON lp.id = ph.path_id
       WHERE lp.user_id = ?
       ORDER BY ph.phase_number ASC`, [id]),
    qAll<any>(
      `SELECT p.course_id, p.phase_id, p.status, p.started_at, p.completed_at,
              c.title, c.provider, c.url, c.duration_hours, c.language, c.rating
       FROM siswa_course_progress p
       JOIN course_catalog c ON c.id = p.course_id
       WHERE p.user_id = ?
       ORDER BY p.phase_id, p.status, c.rating DESC`, [id]),
    qAll<any>(
      `SELECT activity_type, ref_id, created_at FROM siswa_activity_log
       WHERE user_id = ? ORDER BY created_at DESC LIMIT 100`, [id]),
    qAll<any>(
      `SELECT badge_code, awarded_at, meta FROM siswa_badges
       WHERE user_id = ? ORDER BY awarded_at DESC`, [id]),
  ]);
